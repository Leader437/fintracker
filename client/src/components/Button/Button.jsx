...
const Button = ({
  ...
  return (
    <button type={type} className={`button font-display ${bgColor} ${textColor} ${className} ${category==="secondary"? "button-secondary" : ""}`} {...props}>
      <span className="shadow" />
      <span className="edge" />
      <div className={`front text-${size}`}>
        <span className="flex items-center gap-1 whitespace-nowrap">{children}</span>
      </div>
    </button>
  );
};
